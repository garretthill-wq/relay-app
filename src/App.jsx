...
const TEAM_B_MEMBERS = ["Alex", "Jordan", "Sam", "Riley", "Morgan"];

const STATUS_CONFIG = {
  "Incoming":    { color: "#94a3b8", bg: "#1e293b" },
...
const sampleRequests = [
  {
    ...
    status: "In Progress",
    assignedTo: "Alex",
    teamNotes: [{ author: "Alex", text: "Drafted. Sending for legal review before publish.", time: "1 hour ago" }],
...
export default function WorkflowTool() {
  ...
  const handleSubmit = () => {
    ...
    const newReq = { id: idCounter, ...f, attachments, status: "Incoming", assignedTo: "", teamNotes: [],
  ...
  const filteredRequests = requests
    ...
    .sort((a, b) => {
      ...
      if (sortBy === "due") return new Date(a.effectiveDate || "9999") - new Date(b.effectiveDate || "9999");
      return 0;
    });
    ...
    <div className="fade-in" style={{ padding: "32px" }}>
      <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", marginBottom: 24 }}>
        <div>
          ...
          <p style={{ color: "#475569", fontSize: 13 }}>{filteredRequests.length} requests · Team B workspace</p>
        </div>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 18, flexWrap: "wrap" }}>
        ...
        <select value={sortBy} onChange={e => setSortBy(e.target.value)} style={{ ...selectStyle, width: "auto", minWidth: 130 }}>
          <option value="newest">Newest first</option>
          <option value="priority">By priority</option>
          <option value="due">By effective date</option>
        </select>
      </div>
            ...
            >
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                {req.assignedTo ? (
                  ...
                  <div style={{ width: 28, height: 28, borderRadius: "50%", border: "2px dashed #334155", display: "flex", alignItems: "center", justifyContent: "center", color: "#334155", fontSize: 13 }}>?</div>
                )}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            <div>
              <div style={{ display: "flex", gap: 8, marginBottom: 8, flexWrap: "wrap" }}>
                <span style={{ fontSize: 11, color: "#334155", fontFamily: "monospace" }}>#{req.id}</span>
                <span style={{ background: statusCfg.bg, color: statusCfg.color, border: `1px solid ${statusCfg.color}44`, borderRadius: 5, padding: "2px 8px", fontSize: 11, fontWeight: 600 }}>{req.status}</span>
              </div>
              <h2 style={{ fontSize: 19, fontWeight: 700, color: "#f8fafc", letterSpacing: "-0.02em", marginBottom: 4 }}>{req.item}</h2>
              <p style={{ fontSize: 12, color: "#475569" }}>
                Submitted by <span style={{ color: "#94a3b8" }}>{req.submitterName}</span> · {req.submittedAt} · {req.reason} · {req.contentType}
              </p>
            </div>
    ...
    <div style={{ minHeight: "100vh", background: "#0a0a0f", fontFamily: "'DM Sans', 'Segoe UI', sans-serif", color: "#e2e8f0" }}>
      ...
      <div style={{ borderBottom: "1px solid #1a1a2e", padding: "0 32px", display: "flex", alignItems: "center", justifyContent: "space-between", height: 58, background: "#080810", position: "sticky", top: 0, zIndex: 100 }}>
        ...
        <div style={{ display: "flex", gap: 4 }}>
          {[["submit", "📬 Submit Request", "Team A"], ["tracker", "📋 Tracker", "Team B"]].map(([v, label, team]) => (
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
          <div style={{ width: 24, height: 24, borderRadius: "50%", background: "linear-gradient(135deg, #f59e0b, #ef4444)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: 700, color: "white" }}>
            {requests.filter(r => r.status === "Incoming").length}
          </div>
